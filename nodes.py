...
IMMEDIATE (0-tick latency):
  sell_surplus          params: {zone_id, amount: float, rate: float=0.50}         cost: 0
...
NOTES:
  ...
  - sell_surplus GENERATES income; rate=0.5 means 1 unit energy = 0.5 budget
"""
...
class ZoneAgentNodes:
    ...
    def __init__(self, engine, zone_id: str, llm_model: str = "gpt-4o"):
        ...
        # Use ChatOpenAI directly for OpenAI models
        self.llm = ChatOpenAI(
            model=llm_model,
            max_tokens=2048,
            temperature=0.2,    
    ...
    def report(self, state: AgentState) -> dict:
        ...
        print(f"\n  ╔{'═'*62}╗")
        print(f"  ║  AI REPORT — Tick {tick:>3} | Zone {zone:<8}"
              + " " * max(0, 34 - len(state['zone_id'])) + "║")
        print(f"  ╠{'═'*62}╣")
