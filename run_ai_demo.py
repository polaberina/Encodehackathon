...
# ── Agent imports ─────────────────────────────────────────────────────────────
from agents.graph import create_zone_graph
...
def register_crises(engine: SimulationEngine):
    ...
    crises = [
        ...
        (lambda: CrisisEvent(
            ...
            parameters={"output_modifier": 0.40},
            description="Drought: Beta hydro at 40% for 6 ticks",
        ), 5, 0.90),
        ...
        (lambda: CrisisEvent(
            ...
            parameters={"demand_modifier": 1.25, "solar_output_modifier": 0.80},
            description="Heat wave: Gamma demand +25%, solar at 80%",
        ), 11, 0.85),
        ...
        (lambda: CrisisEvent(
            ...
            parameters={"leak_rate": 0.03},
            description="Beta storage breach: 3% drain per tick",
        ), 15, 0.65),
        ...
        (lambda: CrisisEvent(
            ...
            parameters={"region": "north", "demand_modifier": 1.50},
            description="North region blackout: +50% demand surge in Alpha + Beta",
        ), 19, 0.80),

        (lambda: CrisisEvent(
            crisis_id="c010", crisis_type=CrisisType.ECONOMIC_RECESSION,
            target_id="gamma", duration=4, remaining_ticks=4,
            parameters={"income_modifier": 0.65},
            description="Economic recession: Gamma income at 65%",
        ), 22, 0.90),
...
def parse_args():
    ...
    parser.add_argument("--model",     type=str,   default="claude-sonnet-4-6",
                        help="Anthropic model for LLM agents")
